...
class BaseAWSObject(object):
    def __init__(self, title=None, tem=None, validation=True, **kwargs):
        self.title = title
        self.tem = tem
        ...
        # Bound it to tem if we know it
        if self.tem is not None:
            self.tem.add_resource(self)
...
class GenericHelperFn(AWSHelperFn):
    """ Used as a fallback for the tem generator """
...
class Template(object):
    ...
    def add_azure_param(self, name, parameter):
        if len(self.azure_parameters) >= MAX_PARAMETERS:
            raise ValueError('Maximum parameter %d reached' % MAX_PARAMETERS)
        self.azure_parameters[name] = parameter

    def add_azure_variables(self, name, variables):
        if len(self.variables) >= MAX_PARAMETERS:
            raise ValueError('Maximum variables %d reached' % MAX_PARAMETERS)
    ...
    def to_dict(self):
        ...
        if self.azure_parameters:
            t['parameters'] = self.azure_parameters
        if self.variables:
